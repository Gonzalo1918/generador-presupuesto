...
def main(page: ft.Page):
    ...
    page.scroll = "adaptive"
    page.padding = 16
    # permitir tamaños pequeños pero evitar colapso extremo en desktop
    page.window_min_width = 360

    items = []
    edit_index = None
    logo_path = None
    primary = "#A81A1A"  # bordó
    dark_bg = "#0f1112"   # fondo de página en modo oscuro
    card_dark_bg = "#1e1e1e"  # fondo de la tarjeta en modo oscuro (solicitado)
    card_light_bg = "#ffffff"  # fondo de la tarjeta en modo claro
    max_width = 900  # ancho máximo centrado

    # --- Campos (iniciales sin width fijo; usaremos expand para responsividad) ---
    def make_field(field):
        # common styling hook (no cambios de color ni comportamiento)
        field.border_color = primary if page.theme_mode == "dark" else None
        return field

    empresa = make_field(ft.TextField(label="Empresa", value="Ayrton Neumáticos", expand=True))
    cuit = make_field(ft.TextField(label="CUIT", value="", expand=True))
    direccion = make_field(ft.TextField(label="Dirección", value="Córdoba", expand=True))
    correo = make_field(ft.TextField(label="Correo electrónico", value="ayrtonneumaticos@gmail.com", expand=True))
    cliente = make_field(ft.TextField(label="Cliente", expand=True))
    tipo_id = make_field(ft.Dropdown(label="Tipo ID", options=[ft.dropdown.Option(x) for x in ["CUIT", "DNI", "CUIL"]], value="CUIT"))
    numero_id = make_field(ft.TextField(label="Número ID"))
    descripcion = make_field(ft.TextField(label="Descripción / Nota", expand=True, multiline=True, max_lines=4))
    fecha_emision = make_field(ft.TextField(label="Fecha de emisión", value=datetime.date.today().strftime("%d/%m/%Y")))
    fecha_validez = make_field(ft.TextField(label="Válido hasta", value=(datetime.date.today() + datetime.timedelta(days=7)).strftime("%d/%m/%Y")))
    iva_input = make_field(ft.TextField(label="IVA %", value="0"))

    moneda_dropdown = ft.Dropdown(label="Moneda", options=[ft.dropdown.Option("ARS"), ft.dropdown.Option("USD")], value="ARS")
    cotizacion_input = make_field(ft.TextField(label="Cotización USD (ARS)", value="0"))
    cotizacion_input.visible = False
    ...
    # Logo (no visible hasta cargar)
    logo_preview = ft.Image(width=120, height=100, fit=ft.ImageFit.CONTAIN, visible=False)
    def on_logo_pick(e: ft.FilePickerResultEvent):
        nonlocal logo_path
        if e.files:
            logo_path = e.files[0].path
    ...
    # Ítems
    tabla = ft.Column(scroll="auto", expand=True)
    desc_item = make_field(ft.TextField(label="Descripción", expand=True))
    cant_item = make_field(ft.TextField(label="Cantidad", expand=True))
    precio_item = make_field(ft.TextField(label="Precio Unitario (ARS)", expand=True))
    desc_tipo_item = make_field(ft.Dropdown(label="Tipo desc.", expand=True, options=[ft.dropdown.Option("%"), ft.dropdown.Option("$")], value="$"))
    desc_val_item = make_field(ft.TextField(label="Valor desc.", value="0", expand=True))
    ...
    def render_tabla():
        ...
        for idx, it in enumerate(items):
            fila = ft.Row([
                ft.Container(ft.Text(it["descripcion"], overflow=ft.TextOverflow.ELLIPSIS), expand=True),
                ft.Container(ft.Text(str(it["cantidad"])), width=60),
                ft.Container(ft.Text(format_currency_ar(it["precio"])), width=120),
                ft.Container(ft.Text(f"{it['valor_desc']}{it['tipo_desc']}"), width=70),
                ft.Container(ft.Text(format_currency_ar(it["subtotal"])), width=110),
                ft.IconButton(icon=ft.Icons.EDIT, icon_color=primary, on_click=lambda e, i=idx: editar_item(i)),
                ft.IconButton(icon=ft.Icons.DELETE, icon_color="#666", on_click=lambda e, i=idx: eliminar_item(i))
    ...
    subtotal_txt = ft.Text()
    desc_txt = ft.Text()
    iva_txt = ft.Text()
    total_txt = ft.Text(size=18, weight="bold", color=primary)
    equiv_txt = ft.Text()
    ...
    def generar_pdf_click(e):
        ...
        archivo = generar_pdf(
            empresa.value, cuit.value, direccion.value, correo.value, (logo_preview.src[7:] if logo_preview.visible and logo_preview.src else None),
            nro, fecha_emision.value, fecha_validez.value, cliente.value,
            tipo_id.value, numero_id.value, descripcion.value, items,
            subtotal_general, descuento_total, iva_val, total_general,
            mostrar_logo.value, mostrar_cuit.value, mostrar_dir.value, mostrar_correo.value,
    ...
    def toggle_tema(e):
        if page.theme_mode == "light":
            ...
            page.bgcolor = "#f9f9f9"
            theme_btn.icon = ft.Icons.LIGHT_MODE
        # actualizo bordes de inputs
        for f in [empresa, cuit, direccion, correo, cliente, tipo_id, numero_id,
                  descripcion, fecha_emision, fecha_validez, iva_input,
                  desc_item, cant_item, precio_item, desc_val_item, desc_tipo_item]:
            f.border_color = primary if page.theme_mode == "dark" else None
        # actualizo color del container en rebuild_layout (se hará visible)
        rebuild_layout()
        page.update()

    theme_btn.on_click = toggle_tema

    # --- Layout builder (rebuilds according to page.width) ---
    # Container central (tarjeta) - su fondo cambia según tema
    contenido = ft.Container(padding=20, border_radius=12,
                             shadow=ft.BoxShadow(blur_radius=12, spread_radius=1, color="#333"),
                             width=max_width)

    def rebuild_layout(e: ft.ControlEvent = None):
        ancho = page.width or 900
        # card width: never wider than max_width, leave margins on the sides and always allow vertical stacking
        container_width = min(max_width, max(340, ancho - 80))
        contenido.width = container_width

        # set card background according to theme
        contenido.bgcolor = card_dark_bg if page.theme_mode == "dark" else card_light_bg

        title_size = 22
        # header with theme button placed in a small container to avoid overflow
        header = ft.Row([
            ft.Text("Generador de Presupuestos", size=title_size, weight="bold"),
            ft.Container(expand=True),
            ft.Container(theme_btn, padding=ft.padding.only(left=6, right=2))
        ], vertical_alignment=ft.CrossAxisAlignment.CENTER)

        # Build vertical layout (always stacked)
        # Empresa block
        empresa_block = ft.Column([empresa, cuit], tight=True)

        # Logo controls (kept simple, stacked)
        logo_controls = ft.Column([ft.Row([btn_logo, ft.Container(width=12), logo_preview])], tight=True)

        # Switches row (wrap if necessary)
        switches = ft.Row([mostrar_logo, mostrar_cuit, mostrar_dir, mostrar_correo], wrap=True)

        # Cliente block - Tipo ID above Número ID ALWAYS
        cliente_block = ft.Column([
            cliente,
            tipo_id,
            numero_id
        ], tight=True)
        ...
        # Dates block (stacked)
        fechas_block = ft.Column([
            fecha_emision, fecha_validez, iva_input, moneda_dropdown, cotizacion_input
        ], tight=True)

        # Items input - ALWAYS vertical stacking as requested:
        item_input_block = ft.Column([
            desc_item,
            cant_item,        # cantidad
            precio_item,      # precio unitario *DEBAJO* de cantidad (siempre)
            desc_tipo_item,   # tipo de descuento
            desc_val_item,    # valor de descuento *DEBAJO* de tipo (siempre)
            ft.Row([btn_agregar])
        ], tight=True)

        # Totals (right aligned inside card, but stacked in card)
        totals_col = ft.Column([subtotal_txt, desc_txt, iva_txt, equiv_txt, ft.Divider(), total_txt],
                               horizontal_alignment=ft.CrossAxisAlignment.END)

        totals_container = ft.Container(totals_col, padding=10, border_radius=6, alignment=ft.alignment.center_right)

        # Tabla container
        tabla_container = ft.Container(tabla, expand=True, height=220)

        # Bottom buttons stacked (expand)
        botones_finales = ft.Column([
            ft.ElevatedButton("Generar y abrir PDF", on_click=generar_pdf_click, bgcolor=primary, color="white", expand=True),
            ft.Container(height=8),
            ft.ElevatedButton("Limpiar", on_click=lambda e: (items.clear(), render_tabla()), bgcolor="#666", color="white", expand=True)
        ], tight=True)

        # assemble controls in the same order you used originally
        controls = [
            header,
            ft.Divider(),
            ft.Text("Datos de la empresa", size=16, weight="bold", color=primary),
        ...
        contenido.content = ft.Column(controls, scroll="adaptive", spacing=12)

        # Rebuild page layout centered horizontally
        page.controls.clear()
        page.add(
            ft.Row([
                ft.Container(expand=True),
        ...
        # theme backgrounds & neutral gray shadow
        if page.theme_mode == "dark":
            page.bgcolor = dark_bg
            contenido.bgcolor = card_dark_bg
            contenido.shadow = ft.BoxShadow(blur_radius=8, spread_radius=1, color="#222222")
        else:
            page.bgcolor = "#f9f9f9"
            contenido.bgcolor = card_light_bg
            contenido.shadow = ft.BoxShadow(blur_radius=12, spread_radius=1, color="#333333")
    ...
    # resize handler => reconstruye layout en cada cambio de tamaño (responsive)
    def on_resize(e):
        rebuild_layout(e)
    page.on_resize = on_resize

    # Añadimos listener al cambio de tema (por si se cambia desde sistema)
    page.on_theme_change = lambda e: rebuild_layout()

    # Mostrar contenido inicial (ya lo agrega rebuild_layout)
    # end main
